# SPEC-28 — Personalized Album Builder

## Objective

- Implement guided custom album creation.
...
- Create AlbumOrder draft/configuration model.
- Validate selected photos belong to customer.
- Calculate price server-side.
- Persist personalization data.

## Frontend Scope

- Flow: type → size → cover/material → color → pages → photo selection → personalization → review.
- Support child name/title/date/cover text where configured.
...
- [ ] Customer can resume/review configured album where supported.
- [ ] Price is backend-controlled.
