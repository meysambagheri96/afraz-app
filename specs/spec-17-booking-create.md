...
- Validate service/package/date/time/customer data.
- Calculate backend price/deposit.
- Prevent double booking using DB constraints/transaction.
- Create booking in pending/appropriate state.

## Frontend Scope

- Implement booking steps: service → package → date → time → customer info → review.
