# Use Infra for CQRS and DDD foundations
...
`IQueryHandler<TQuery, TResult>`. HTTP endpoints dispatch through `ICommandProcessor` or
`IQueryProcessor` and remain thin.

## Consequences

- Application slices depend on the `Infra` CQRS contracts.
- The API composition root uses Autofac in addition to the standard ASP.NET Core service
  registrations.
...
- Keep MediatR: rejected because it would not match the requested internal platform conventions.
- Run MediatR and Infra side by side: rejected because two dispatch pipelines make validation,
  logging, and handler resolution ambiguous.
- Copy every dependency from the template: rejected because Kafka and other infrastructure are not
  needed for the current foundation and would violate the modular-monolith scope discipline.
